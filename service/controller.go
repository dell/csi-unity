...
func (s *service) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if arrayID == "" {
		return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "ArrayId cannot be empty"))
	}
	...
	if protocol == NFS {
		...
		if !ok {
			return nil, status.Errorf(codes.InvalidArgument, "%s", serviceutils.GetMessageWithRunID(rid, "`%s` is a required parameter", keyNasServer))
		}
		...
		if filesystem != nil {
			...
			if int64(content.SizeTotal) /* #nosec G115 -- This is a false positive */ == size && content.NASServer.ID == nasServer && content.Pool.ID == storagePool {
				...
				return serviceutils.GetVolumeResponseFromFilesystem(filesystem, arrayID, protocol, preferredAccessibility), nil
			}
			log.Info("'Filesystem name' already exists and size/NAS server/storage pool is different")
			return nil, status.Error(codes.AlreadyExists, serviceutils.GetMessageWithRunID(rid, "'Filesystem name' already exists and size/NAS server/storage pool is different."))
		...
		if err != nil {
			...
			return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Create Filesystem %s failed with error: %v", volName, err))
		}
		...
		if resp != nil {
			...
			filesystemResp := serviceutils.GetVolumeResponseFromFilesystem(resp, arrayID, protocol, preferredAccessibility)
		...
		if hostIOLimitName != "" {
			...
			if err != nil {
				return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "HostIOLimitName %s not found. Error: %v", hostIOLimitName, err))
			}
		...
		if vol != nil {
			...
			if int64(content.SizeTotal) /* #nosec G115 -- This is a false positive */ == size {
				...
				return serviceutils.GetVolumeResponseFromVolume(vol, arrayID, protocol, preferredAccessibility), nil
			}
			log.Info("'Volume name' already exists and size is different")
			return nil, status.Error(codes.AlreadyExists, serviceutils.GetMessageWithRunID(rid, "'Volume name' already exists and size is different."))
		}
		...
		if err != nil {
			return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Create Volume %s failed with error: %v", volName, err))
		}

		resp, err = unity.FindVolumeByName(ctx, volName)
		if resp != nil {
			volumeResp := serviceutils.GetVolumeResponseFromVolume(resp, arrayID, protocol, preferredAccessibility)
			log.Debugf("CreateVolume successful for volid: [%s]", volumeResp.Volume.VolumeId)
			return volumeResp, nil
		}
	}

	return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Volume/Filesystem not found after create. %v", err))
}
...
func (s *service) DeleteVolume(
	...
	*csi.DeleteVolumeResponse, error,
	...
	return nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "Delete Volume %s failed with error: %v", volID, err))
}
...
func (s *service) ControllerUnpublishVolume(
	...
	*csi.ControllerUnpublishVolumeResponse, error,
	...
	if nodeID == "" {
		return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "Node ID is required"))
	}
	...
	if protocol != NFS {
		...
		if err != nil {
			...
			return nil, status.Error(codes.Internal, serviceutils.GetMessageWithRunID(rid, "%v", err))
		}
		...
		if len(content.HostAccessResponse) > 0 {
			...
			if err != nil {
				return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Unexport Volume Failed. %v", err))
			}
		} else {
...
func (s *service) ValidateVolumeCapabilities(ctx context.Context, req *csi.ValidateVolumeCapabilitiesRequest) (*csi.ValidateVolumeCapabilitiesResponse, error) {
	...
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Volume not found. Error: %v", err))
	}
	...
	if protocol == "" {
		...
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Protocol is required to validate capabilities"))
	}
	...
	resp.Message = reason
	return resp, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Unsupported capability"))
}
...
func (s *service) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
	...
	if len(req.SourceVolumeId) == 0 {
		return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "Storage Resource ID cannot be empty"))
	}
	var err error
	req.Name, err = util.ValidateResourceName(req.Name, api.MaxResourceNameLength)
	if err != nil {
		return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "invalid snapshot name [%v]", err))
	}
	...
	return serviceutils.GetSnapshotResponseFromSnapshot(snap, protocol, arrayID), nil
}
...
func (s *service) DeleteSnapshot(ctx context.Context, req *csi.DeleteSnapshotRequest) (*csi.DeleteSnapshotResponse, error) {
	...
	if snap != nil {
		...
		if err != nil {
			return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Delete Snapshot error: %v", err))
		}
	}
...
func (s *service) ListSnapshots(ctx context.Context, req *csi.ListSnapshotsRequest) (*csi.ListSnapshotsResponse, error) {
	...
	if req.StartingToken != "" {
		...
		if err != nil {
			return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "Unable to parse StartingToken: %v into uint32", req.StartingToken))
		}
		startToken = int(i)
	}
	...
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Unable to get the snapshots: %v", err))
	}
	...
	if err != nil {
		return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "%s", err.Error()))
	}
...
func (s *service) ControllerExpandVolume(ctx context.Context, req *csi.ControllerExpandVolumeRequest) (*csi.ControllerExpandVolumeResponse, error) {
	...
	if req.VolumeId == "" {
		return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "volumeId is mandatory parameter"))
	}
	...
	if capacity <= 0 {
		return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "Required bytes can not be 0 or less"))
	}
	...
	if protocol == NFS {
		...
		if err != nil {
			...
			if err != nil {
				return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Find filesystem %s failed with error: %v", volID, err))
			}
			return nil, status.Error(codes.Unimplemented, serviceutils.GetMessageWithRunID(rid, "Expand Volume not supported for cloned filesystems(snapshot on array)"))
		}
		...
		if err != nil {
			return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Expand filesystem failed with error: %v", err))
		}

		filesystem, err = unity.FindFilesystemByID(ctx, volID)
		if err != nil {
			return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Find filesystem failed with error: %v", err))
		}
	...
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Find volume failed with error: %v", err))
	}
	...
	if err != nil {
		return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Expand volume failed with error: %v", err))
	}
...
func (s *service) getFilesystemByResourceID(ctx context.Context, resourceID, arrayID string) (*types.Filesystem, error) {
	...
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Storage resource: %s filesystem Id not found. Error: %v", resourceID, err))
	}
	sourceFilesystemResp, err := unity.FindFilesystemByID(ctx, filesystemID)
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Filesystem: %s not found. Error: %v", filesystemID, err))
	}
...
func (s *service) createFilesystemFromSnapshot(ctx context.Context, snapID, volumeName, arrayID string) (*types.Snapshot, error) {
	...
	if err != nil {
		return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Create Filesystem from snapshot failed with error. Error: %v", err))
	}
...
func (s *service) createIdempotentSnapshot(ctx context.Context, snapshotName, sourceVolID, description, retentionDuration, protocol, arrayID string, isClone bool) (*types.Snapshot, error) {
	...
	if protocol == NFS {
		...
		if err != nil {
			...
			if err != nil {
				return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Find source filesystem: %s failed with error: %v", sourceVolID, err))
			}
			isSnapshot = true
	...
	if snap != nil {
		...
		return nil, status.Error(codes.AlreadyExists, serviceutils.GetMessageWithRunID(rid, "Snapshot with same name %s already exists for storage resource %s", snapshotName, snap.SnapshotContent.StorageResource.ID))
	}
	...
	if isSnapshot {
		...
		if err != nil {
			return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Create Snapshot error: %v", err))
		}
	...
	return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Find Snapshot error after create. %v", err))
}
...
func (s *service) getHostID(ctx context.Context, arrayID, shortHostname, longHostname string) (*types.Host, error) {
	...
	unity, err := s.getUnityClient(ctx, arrayID)
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Unable to get Unity client."))
	}

	host, err := unity.FindHostByName(ctx, shortHostname)
	if err != nil {
		if err != gounity.ErrorHostNotFound {
			return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Find Host Failed %v", err))
		}
	}
	if host != nil {
	...
	return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Find Host Id Failed."))
}
...
func (s *service) createVolumeClone(ctx context.Context, crParams *CRParams, sourceVolID, arrayID string, contentSource *csi.VolumeContentSource, unity gounity.UnityClient, preferredAccessibility []*csi.Topology) (*csi.CreateVolumeResponse, error) {
	ctx, log, rid := GetRunidLog(ctx)
	if sourceVolID == "" {
		return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "Source volume ID cannot be empty"))
	}
	...
	if arrayID != sourceArrayID {
		return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "Source volume array id: %s is different than required volume array id: %s", sourceArrayID, arrayID))
	}
	...
	if protocol == NFS {
		...
		if err != nil {
			...
			if snapErr != nil {
				...
				return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Find source filesystem: %s Failed. Error: %v ", sourceVolID, err))
			}
			isSnapshot = true
		...
		if isSnapshot {
			...
			if snapSize != size {
				return nil, status.Errorf(codes.InvalidArgument, "%s", serviceutils.GetMessageWithRunID(rid, "Requested size %d should be same as source filesystem size %d", size, snapSize))
			}
			// Idempotency check
			...
			if snapResp == nil {
				...
				return nil, status.Error(codes.AlreadyExists, serviceutils.GetMessageWithRunID(rid, "Snapshot with same name %s already exists in different size.", volName))
			}
			snapResp.SnapshotContent.Size -= AdditionalFilesystemSize
			csiVolResp := serviceutils.GetVolumeResponseFromSnapshot(snapResp, arrayID, protocol, preferredAccessibility)
		...
		if size != fsSize {
			return nil, status.Errorf(codes.InvalidArgument, "%s", serviceutils.GetMessageWithRunID(rid, "Requested size %d should be same as source volume size %d",
				size, fsSize))
		}
		...
		csiVolResp := serviceutils.GetVolumeResponseFromSnapshot(snap, arrayID, protocol, preferredAccessibility)
		csiVolResp.Volume.ContentSource = contentSource
		return csiVolResp, nil
	}
	...
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Source volume not found: %s. Error: %v", sourceVolID, err))
	}
	...
	if volResp != nil {
		...
		if volResp.VolumeContent.IsThinClone && len(volResp.VolumeContent.ParentVolume.ID) > 0 && volResp.VolumeContent.ParentVolume.ID == sourceVolID &&
			...
			csiVolResp := serviceutils.GetVolumeResponseFromVolume(volResp, arrayID, protocol, preferredAccessibility)
			csiVolResp.Volume.ContentSource = contentSource
			return csiVolResp, nil
		}
		return nil, status.Error(codes.AlreadyExists, serviceutils.GetMessageWithRunID(rid, "Volume with same name %s already exists", volName))
	}
	...
	if err != nil {
		if err == gounity.ErrorCreateSnapshotFailed {
			return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Unable to Create Snapshot for Volume Cloning for source volume: %s", sourceVolID))
		} else if err == gounity.ErrorCloningFailed {
			return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Volume cloning for source volume: %s failed.", sourceVolID))
		}
	}
	...
	return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Volume not found after create. %v", err))
}
...
func (s *service) createVolumeFromSnap(ctx context.Context, crParams *CRParams, snapshotID, arrayID string, contentSource *csi.VolumeContentSource, unity gounity.UnityClient, preferredAccessibility []*csi.Topology) (*csi.CreateVolumeResponse, error) {
	ctx, log, rid := GetRunidLog(ctx)
	if snapshotID == "" {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Source snapshot ID cannot be empty"))
	}
	...
	if arrayID != sourceArrayID {
		return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "Source snapshot array id: %s is different than required volume array id: %s", sourceArrayID, arrayID))
	}
	...
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Source snapshot not found: %s", snapshotID))
	}
	...
	if protocol == NFS {
		...
		snapSize := int64(snapResp.SnapshotContent.Size - AdditionalFilesystemSize)
		if snapSize != size {
			return nil, status.Errorf(codes.InvalidArgument, "%s", serviceutils.GetMessageWithRunID(rid, "Requested size %d should be same as source snapshot size %d", size, snapSize))
		}
		...
		if snapResp != nil {
			...
			return nil, status.Error(codes.AlreadyExists, serviceutils.GetMessageWithRunID(rid, "Filesystem with same name %s already exists", volName))
		}
		...
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Filesystem: %s not found after create. Error: %v", volName, err))
	}
	...
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Source volume not found: %s", volID))
	}
	...
	if snapResp.SnapshotContent.Size != size {
		return nil, status.Errorf(codes.InvalidArgument, "%s", serviceutils.GetMessageWithRunID(rid, "Requested size %d should be same as source snapshot size %d", size, snapResp.SnapshotContent.Size))
	}
	...
	if snapResp.SnapshotContent.IsAutoDelete == true {
		...
		if err != nil {
			return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Unable to modify auto-delete parameter for snapshot %s", snapshotID))
		}
	}

	volResp, err = unity.CreteLunThinClone(ctx, volName, snapshotID, volID)
	if err != nil {
		return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Create volume from snapshot failed with error %v", err))
	}
	volResp, err = unity.FindVolumeByName(ctx, volName)
	if err != nil {
...
func (s *service) deleteFilesystem(ctx context.Context, volID string, unity gounity.UnityClient) (error, error, error) {
	...
	if err == nil {
		...
		if len(filesystemResp.FileContent.NFSShare) > 0 || len(filesystemResp.FileContent.CIFSShare) > 0 {
			return nil, nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "Filesystem %s can not be deleted as it has associated NFS or SMB shares.", volID))
		}
		snapsResp, _, snapshotErr := unity.ListSnapshots(ctx, 0, 0, filesystemResp.FileContent.StorageResource.ID, "")
		if snapshotErr != nil {
			return nil, nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "List snapshots for filesystem %s failed with error: %v", volID, snapshotErr))
		}

		for _, snapResp := range snapsResp {
			if snapResp.SnapshotContent.AccessType == int(gounity.CheckpointAccessType) {
				return nil, nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "Filesystem %s can not be deleted as it has associated snapshots.", volID))
			}
		}
		...
		if fsSnapErr == nil {
			...
			if err != nil {
				return nil, nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Source storage resource: %s filesystem Id not found. Error: %v", snapResp.SnapshotContent.StorageResource.ID, err))
			}
			filesystemResp, err = unity.FindFilesystemByID(ctx, sourceVolID)
			if err != nil {
				return nil, nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Find source filesystem: %s failed with error: %v", sourceVolID, err))
			}
			for _, nfsShare := range filesystemResp.FileContent.NFSShare {
				if nfsShare.ParentSnap.ID == volID {
					return nil, nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "Snapshot %s can not be deleted as it has associated NFS or SMB shares.", volID))
				}
			}
...
func (s *service) deleteBlockVolume(ctx context.Context, volID string, unity gounity.UnityClient) (error, error) {
	...
	if snapshotErr != nil {
		return nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "List snapshots for volume %s failed with error: %v", volID, snapshotErr))
	}
	...
	for _, snapResp := range snapsResp {
		...
		if strings.Contains(snapshotName, gounity.SnapForClone) {
			...
			if snapshotErr != nil {
				return nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "Volume %s can not be deleted as it has associated snapshots.", volID))
			}
			totalSnaps--
		}
	}
	...
	if err != nil {
		...
		return nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "Failed to fetch volume %s details from storage array.", volID))
	}
...
func checkVolumeUnexportError(err error, volID, rid string, log *logrus.Entry) (error, error) {
	if err == nil {
		...
		return nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "Remove host access for volume timed out."))
		...
		return nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "Remove host access for volume failed."))
	}
...
func checkVolumeDeleteError(err error, volID, rid string, log *logrus.Entry) (error, error) {
	if err == nil {
		...
		return nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "Delete volume from storage array failed."))
		...
		return nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "Delete volume from storage array failed, since it still has host access."))
		...
		return nil, status.Error(codes.FailedPrecondition, serviceutils.GetMessageWithRunID(rid, "Delete volume from storage array timed out."))
	}
...
func (s *service) exportFilesystem(ctx context.Context, volID, hostID, nodeID, arrayID string, unity gounity.UnityClient, pinfo map[string]string, am *csi.VolumeCapability_AccessMode) (*csi.ControllerPublishVolumeResponse, error) {
	...
	if !nfsShareExist {
		if isSnapshot {
			...
			if err != nil {
				return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Create NFS Share failed. Error: %v", err))
			}
	...
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Find NFS Share: %s failed. Error: %v", nfsShareID, err))
	}
	...
	if foundIncompatible {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Host: %s has access on NFS Share: %s with incompatible access mode.", nodeID, nfsShareID))
	}
	if (am.Mode == csi.VolumeCapability_AccessMode_SINGLE_NODE_WRITER || am.Mode == csi.VolumeCapability_AccessMode_SINGLE_NODE_SINGLE_WRITER || am.Mode == csi.VolumeCapability_AccessMode_SINGLE_NODE_MULTI_WRITER) && otherHostsWithAccess > 0 {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Other hosts have access on NFS Share: %s", nfsShareID))
	}
	// Idempotent case
	if foundIdempotent {
	...
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Allocating host %s access to NFS Share failed. Error: %v", nodeID, err))
	}
...
func (s *service) exportVolume(ctx context.Context, protocol, volID, hostID, _, _ string, unity gounity.UnityClient, pinfo map[string]string, host *types.Host, vc *csi.VolumeCapability) (*csi.ControllerPublishVolumeResponse, error) {
	...
	if protocol == FC && len(hostContent.FcInitiators) == 0 {
		return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "Cannot publish volume as protocol in the Storage class is 'FC' but the node has no valid FC initiators"))
	} else if protocol == ISCSI && len(hostContent.IscsiInitiators) == 0 {
		return nil, status.Error(codes.InvalidArgument, serviceutils.GetMessageWithRunID(rid, "Cannot publish volume as protocol in the Storage class is 'iSCSI' but the node has no valid iSCSI initiators"))
	}

	vol, err := unity.FindVolumeByID(ctx, volID)
	if err != nil {
		return nil, status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Find volume Failed %v", err))
	}
	...
	for _, hostaccess := range content.HostAccessResponse {
		...
		if hostAccessID == hostID {
			...
			return nil, status.Error(codes.Aborted, serviceutils.GetMessageWithRunID(rid, "Volume has been published to a different host already."))
		}
	...
	err = unity.ModifyVolumeExport(ctx, volID, hostIDList)
	if err != nil {
		return nil, status.Error(codes.Unknown, serviceutils.GetMessageWithRunID(rid, "Export Volume Failed %v", err))
	}
	log.Debugf("ControllerPublishVolume successful for volid: [%s]", pinfo["volumeContextId"])
	return &csi.ControllerPublishVolumeResponse{PublishContext: pinfo}, nil
}
...
func (s *service) unexportFilesystem(ctx context.Context, volID, hostID, nodeID, volumeContextID, arrayID string, unity gounity.UnityClient) error {
	...
	if err != nil {
		...
		if err != nil {
			...
			return status.Error(codes.Internal, serviceutils.GetMessageWithRunID(rid, "Find filesystem %s failed with error: %v", volID, err))
		}
		isSnapshot = true
		filesystem, err = s.getFilesystemByResourceID(ctx, snapResp.SnapshotContent.StorageResource.ID, arrayID)
		if err != nil {
	...
	nfsShareResp, err := unity.FindNFSShareByID(ctx, nfsShareID)
	if err != nil {
		return status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Find NFS Share: %s failed. Error: %v", nfsShareID, err))
	}
	readOnlyHosts := nfsShareResp.NFSShareContent.ReadOnlyHosts
	readWriteHosts := nfsShareResp.NFSShareContent.ReadWriteHosts
	readOnlyRootHosts := nfsShareResp.NFSShareContent.ReadOnlyRootAccessHosts
	...
	if foundIncompatible {
		return status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Cannot remove host access. Host: %s has access on NFS Share: %s with incompatible access mode.", nodeID, nfsShareID))
	}
	if foundReadOnly {
		if isSnapshot {
			err = unity.ModifyNFSShareCreatedFromSnapshotHostAccess(ctx, nfsShareID, readHostIDList, gounity.ReadOnlyRootAccessType)
	...
	if err != nil {
		return status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Removing host %s access to NFS Share failed. Error: %v", nodeID, err))
	}
	...
	if deleteShare {
		if otherHostsWithAccess > 0 {
			...
			if err != nil {
				return status.Error(codes.NotFound, serviceutils.GetMessageWithRunID(rid, "Delete NFS Share: %s Failed with error: %v", nfsShareID, err))
			}
